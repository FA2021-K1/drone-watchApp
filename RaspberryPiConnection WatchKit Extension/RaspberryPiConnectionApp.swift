...
struct RaspberryPiConnectionApp: App {
    ...
    var wifiConnectivity = WifiConnectivity(
        ...
        lab: Lab(ssid: "LS1 FA", password: "ls1.internet", url: URL(string: "http://192.168.1.199:6969/v1/measurements/test")!))
    var receivedData = ReceivedData()
    var bleConnectivity = BluetoothConnectivity()
    
    
    init() {
     wifiConnectivity.checkForCurrentNetwork(waitForDisconnect: false)
       bleConnectivity.initBluetooth()
    }
        ...
        WindowGroup {
            NavigationView {
                ContentView()
                    ...
                    .environmentObject(bleConnectivity)
            }
        }
